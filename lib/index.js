...
module.exports = function EnterpriseProbePlugin () {
  ...
  this.init = function (config, context, isDummy) {
    ...
    return createMeasuresIndex(this.client, this.index)
      .then(() => getMissingCollections(this.client, this.index, this.probes))
      .then(missingCollections => startProbes(this.client, this.index, this.probes, this.measures, missingCollections));
  };
...
function getMissingCollections(esClient, index, probes) {
  ...
  return esClient.indices.getMapping({index, type: collections})
    .then(function (response) {
      ...
      return q(inexistantCollections);
    });
}
 ...
 * @param {Array} missingCollections
 */
function startProbes(client, index, probes, measures, missingCollections) {
      ...
      createCollection(client, index, probes[name], missingCollections)
        .then(() => {
          if (probes[name].interval) {
            console.log(longTimeout.setInterval.toString());
