  ...
  monitor (request) {
    ...
    this.eventMapping.monitor[event].forEach(probe => {
      ...
      if (!this.probes[probe].interval) {
        afterMeasure(this.context, this.index, this.probes[probe], this.measures[probe]);
      }
    });
  ...
  watcher (request) {
    ...
    matchedIds.forEach(filterId => {
      this.eventMapping.watcher[filterId].forEach(name => {
        ...
        if (!probe.interval) {
          afterMeasure(this.context, this.index, probe, this.measures[name]);
        }
      });
    });
              ...
              .then(() => {
                if (this.probes[name].interval) {
                  longTimeout.setInterval(
                    () => afterMeasure(this.context, this.index, this.probes[name], this.measures[name]),
...
function createMeasuresIndex(context, index) {
  return context.accessors.execute(
...
function getMissingCollections(context, index, probes) {
  ...
  return context.accessors.execute(context.constructors.Request({
    index,
...
function createCollection(context, measureIndex, probe, missingCollections) {
  ...
  const
    ...
    creationRequest = context.constructors.Request({
      index: measureIndex,
  ...
  mappingRequest = context.constructors.Request({
    index: measureIndex,
    collection: probe.name,
    controller: 'collection',
...
function saveMeasure(context, index, probe, measure) {
  ...
  if (measure.content) {
    ...
    promise = context.accessors.execute(context.constructors.Request({
      index,
      collection: probe.name,
      controller: 'bulk',
      action: 'import',
 ...
 * @param {Object} measure
 */
function afterMeasure(context, index, probe, measure) {
  return new Promise((resolve, reject) => {
    if (!probe.volatile) {
      saveMeasure(context, index, probe, measure)
        .then(() => resolve())
    ...
    notifyMeasure(context, probe, measure);
 ...
 * @param {Object} measure
 */
function notifyMeasure(context, probe, measure) {
  context.accessors.trigger('receivedMeasure', {
