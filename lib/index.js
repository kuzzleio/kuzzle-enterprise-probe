...
class EnterpriseProbePlugin {
   ...
   * @returns {Promise}
   */
  init (customConfig, context, isDummy) {
    const
      defaultConfig = {
        threads: 1,
        loadedBy: 'server',
   ...
   */
  watcher (request) {
    this.dsl.test(request.input.resource.index, request.input.resource.collection, request.input.body, request.input.resource._id)
      .then(matchedIds => {
        matchedIds.forEach(filterId => {
          this.eventMapping.watcher[filterId].forEach(name => {
            ...
            if (probe.collects) {
              this.measures[name].content.push(collectData(request.input.resource._id, request.input.resource.body, probe.collects));
            }
            else {
...
function getMissingCollections(esClient, index, probes) {
  ...
  return esClient.indices.getMapping({index, type: collections})
    .then(function (response) {
      ...
      return Bluebird.resolve(inexistantCollections);
    });
}
 ...
 * @returns {Promise}
 */
function createCollection(esClient, measureIndex, probe, missingCollections) {
...
function startProbes(client, index, probes, measures) {
  return getMissingCollections(client, index, probes)
    .then(missingCollections => {
      ...
      Object.keys(probes)
        .forEach(name => {
          ...
          createCollection(client, index, probes[name], missingCollections)
            .then(() => {
              if (probes[name].interval) {
