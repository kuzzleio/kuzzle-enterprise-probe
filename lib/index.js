...
function configureProbes(probes) {
  ...
  Object.keys(output).forEach(name => {
    ...
    /*
     watcher and sampler probes need these required parameters
      index
      collection
     */
    if (['watcher', 'sampler'].indexOf(output[name].type) > -1) {
      if (!output[name].index || !output[name].collection) {
        console.error(`plugin-probe: [probe: ${name}] Configuration error: missing index or collection`);
        console.error('Removing probe from list.');
        return delete output[name];
      }

      if (output[name].filter === undefined || output[name].filter === null) {
        output[name].filter = {};
      }

      // we also need to set up an unique filter ID. Thanksfully, Kuzzle DSL contains an id generator
      output[name].filterId = this.dsl.createFilterId(output[name].index, output[name].collection, output[name].filter);
    }
  });
