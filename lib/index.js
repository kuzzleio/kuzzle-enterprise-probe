...
class ProbePlugin {
  ...
  _configureProbes(probes) {
    ...
    Object.keys(probes).forEach(name => {
      ...
      if (!probe.type) {
        this.context.log.error(`plugin-probe: [probe: ${name}] "type" parameter missing"`);
        return;
      }

      if (probe.type === 'sampler' && (probe.interval === 'none' || !probe.interval)) {
        this.context.log.error(`plugin-probe: [probe: ${name}] An "interval" parameter is required for sampler probes`);
        return;
      }
      ...
      if (probe.interval === 'none') {
        ...
        if (isNaN(probe.interval)) {
          this.context.log.error(`plugin-probe: [probe: ${name}] Invalid interval "${probe.interval}".`);
          return;
        }
      }
      ...
      if (probe.type === 'counter' && _.intersection(probe.increasers, probe.decreasers).length > 0) {
        this.context.log.error(`plugin-probe: [probe: ${name}] Configuration error: an event cannot be set both to increase and to decrease a counter`);
        return;
      }
      ...
      if (['watcher', 'sampler'].indexOf(probe.type) > -1) {
        if (!probe.index || !probe.collection) {
          this.context.log.error(`plugin-probe: [probe: ${name}] Configuration error: missing index or collection`);
          return;
        }
        ...
        if (probe.collects) {
          if (typeof probe.collects !== 'string' && !Array.isArray(probe.collects)) {
            this.context.log.error(`plugin-probe: [probe: ${name}] Invalid "collects" format: expected array or string, got ${typeof probe.collects}`);
            return;
          }

          if (typeof probe.collects === 'string' && probe.collects !== '*') {
            this.context.log.error(`plugin-probe: [probe: ${name}] Invalid "collects" value`);
            return;
          }
        ...
        if (probe.type === 'sampler' && !probe.collects) {
          this.context.log.error(`plugin-probe: [probe: ${name}] A "collects" parameter is required for sampler probes`);
          return;
        }
      ...
      if (probe.type === 'sampler') {
        if (!probe.sampleSize) {
          this.context.log.error(`plugin-probe: [probe: ${name}] "sampleSize" parameter missing`);
          return;
        }

        if (typeof probe.sampleSize !== 'number') {
          this.context.log.error(`plugin-probe: [probe: ${name}] invalid "sampleSize" parameter. Expected a number, got a ${typeof probe.sampleSize}`);
          return;
        }
      }
  ...
  _afterMeasure(probe, measure) {
    return new Promise((resolve, reject) => {
      ...
      this._showError(probe, measure, err);
    });
  }
