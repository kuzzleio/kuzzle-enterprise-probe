...
module.exports = function EnterpriseProbePlugin () {
  ...
  this.init = function (config, context, isDummy) {
    ...
    registerInDsl(this.dsl, this.probes);
  ...
  /**
   * Watcher probe
   * Watch documents and messages activity, counting them and/or retrieving part of their content.
   ...
   * - a '*' string value: the entire **flattened** document/message will be collected
   * - an array listing the document/message attributes to collect
   *
   * The "filters" parameter configures what documents/messages will be watched.
      ...
      .then(matchedIds => {
        matchedIds.forEach(filterId => {
          this.eventMapping.watcher[filterId].forEach(name => {
            ...
            if (probe.collects) {
              flatContent = flattenContent(requestObject.data.body, requestObject.data._id);

              if (probe.collects === '*') {
                collected = flatContent;
              }
              else {
                collected._id = flatContent._id;

                probe.collects.forEach(attribute => {
                  collected[attribute] = flatContent[attribute];
                });
              }

              this.measures[name].content.push(collected);
            }
            else {
 ...
 *    },
 *    watcher: {
 *      filterUniqueId: [associated, probes, list],
 *      ...
 *    }
 *  }
 *
...
function configureProbes(probes) {
  ...
  Object.keys(probes).forEach(name => {
     ...
     */
    if (['watcher', 'sampler'].indexOf(output[name].type) > -1) {
      ...
      // we also need to set up an unique filter ID. Thankfully, Kuzzle DSL contains an id generator
      probe.filterId = this.dsl.createFilterId(probe.index, probe.collection, probe.filter);
    }
...
function saveMeasure(client, index, probe, measure) {
  ...
  if (measure.content) {
    ...
    promise = client.bulk(bulkRequest);
  }
  else {
 ...
 *
 * @param {Object} dsl instance
 * @param {Object} probes
 */
function registerInDsl(dsl, probes) {
  Object.keys(probes).forEach(name => {
    var probe = probes[name];

    if (['watcher', 'sampler'].indexOf(probe.type) > -1) {
      dsl.register(probe.filterId, probe.index, probe.collection, probe.filters);
    }
  });
}

/**
 * Transforms an object content:
 * {
 *  title: "kuzzle",
 *  info : {
 *    tag: "news"
 ...
 */
function flattenContent(target, id) {
  var
    delimiter = '.',
    output = {};
