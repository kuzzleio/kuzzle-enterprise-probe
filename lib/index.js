...
module.exports = function EnterpriseProbePlugin () {
  ...
  this.init = function (config, context, isDummy) {
    ...
    this.dsl = registerInDsl(context, this.probes);
      ...
      .then(matchedIds => {
        matchedIds.forEach(filterId => {
          this.eventMapping.watcher[filterId].forEach(name => {
            ...
            if (probe.collects) {
              if (probe.collects === '*') {
                collected = requestObject.data.body;
              }
              else {
                probe.collects.forEach(field => _.set(collected, field, _.get(requestObject.data.body, field)));
              }
/*
              if (probe.collects === '*') {
                collected = requestObject.data.body;
...
*/
              collected._id = requestObject.data._id;
...
function saveMeasure(client, index, probe, measure) {
  ...
  if (measure.content) {
    ...
    promise = client.bulk({body: bulkRequest});
  }
  else {
 ...
 *
 * @param {Object} plugin context
 * @param {Object} probes
 * @returns {Object} Instantiated DSL
 */
function registerInDsl(context, probes) {
